...
FastAPI 使用 `APIRouter` 来组织和结构化应用的端点。这对于将 API 拆分到多个文件和模块中特别有用，这是构建更大、可维护应用的常见做法。每个独立的端点在内部由一个 `APIRoute` 表示。

本页面为这些类提供了详细的参考。有关如何使用路由器构建项目结构的分步指南，请参阅[更大型的应用](./advanced-bigger-applications.md)教程。

## `APIRouter`

`APIRouter` 类允许你对*路径操作*进行分组。你可以将其看作一个迷你的 `FastAPI` 应用。你可以在路由器上定义路由、依赖项和标签，然后将其包含在主应用甚至另一个路由器中。
...
当你创建 `APIRouter` 的实例时，可以传递几个参数来配置它包含的所有路由。
...
| `include_in_schema` | `bool` | 如果为 `False`，则将此路由器中的所有*路径操作*从生成的 OpenAPI 模式中排除。默认为 `True`。 |
...
| `lifespan` | `Optional[Lifespan[Any]]` | 用于处理此路由器的启动和关闭事件的 `Lifespan` 上下文管理器。 |

### 路径操作装饰器

`APIRouter` 为所有标准 HTTP 方法提供装饰器，以添加新的*路径操作*。
...
所有这些装饰器都接受与 `APIRoute` 类相同的参数，具体如下所述。
...
# from . import items # 假设 items.py 在同一目录中
...
`APIRoute` 类代表单个*路径操作*。通常你不会直接创建 `APIRoute` 的实例。相反，它们是由 `APIRouter` 的路径操作装饰器（例如 `@router.get(...)`）创建的。此处列出的参数在所有这些装饰器中都可用。

| Parameter | Type | Description |
|---|---|---|
| `path` | `str` | 此操作的 URL 路径。 |
| `endpoint` | `Callable[..., Any]` | 将处理对此路径的请求的函数。 |
| `response_model` | `Any` | 用于响应的 Pydantic 模型。它有助于数据验证、序列化和文档编制。 |
...
了解如何构建路由后，你可能希望深入了解如何定义参数。更多信息，请参阅[参数](./api-reference-parameters.md)参考。