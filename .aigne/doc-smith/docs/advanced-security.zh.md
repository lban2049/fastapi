...
一个典型的身份验证流程，例如使用持有者令牌的 OAuth2，首先涉及客户端使用凭据进行身份验证以获取令牌，然后使用该令牌访问受保护的资源。
...
OAuth2 是一种广泛使用的授权协议。“密码”流程是用户直接提供凭据以换取访问令牌的常用方式。然后，该令牌作为“持有者”令牌在后续请求的 `Authorization` 标头中发送。

### 第一步：创建安全方案

首先，您需要一个 `OAuth2PasswordBearer` 的实例。该对象是一个依赖项，它将在 `Authorization` 标头中要求一个持有者令牌。
...
这样，`/items/` 端点将要求一个 `Authorization` 标头，其值类似于 `Bearer your-token-here`。该依赖项将以 `str` 形式返回令牌。
...
现在，`read_users_me` 路径操作依赖于 `get_current_user`。FastAPI 将按顺序调用这些依赖项：首先是 `oauth2_scheme`，然后是将结果传递给 `get_current_user`。最终结果是一个 `User` 对象。
...
3.  **在依赖项中检查范围：** 必须更新 `get_current_user` 依赖项，以检查令牌是否包含正在访问的端点所需的范围。
...
HTTP 基本身份验证是一种更简单的方案，其中用户名和密码包含在 `Authorization` 标头中，并经过 Base64 编码。虽然简单，但它只应在 HTTPS 上使用，因为凭据并未加密。
...
API 密钥是向特定客户端或服务授予访问权限的常用方法。密钥是客户端在每个请求中发送的单个令牌。FastAPI 提供了从不同位置提取 API 密钥的辅助工具。
...
使用 `APIKeyHeader` 来期望在自定义标头（例如 `x-key`）中接收 API 密钥。
...
我们非常重视安全性。建议您保持 FastAPI 版本为最新，以便从最新的功能、错误修复和安全修复中受益。
...
借助这些工具，您可以在 FastAPI 应用程序中实现强大且标准的安全实践。有关安全实用工具的更多详细信息，您可以查阅 [安全实用工具 API 参考](./api-reference-security.md)。要了解在请求到达路径操作之前的处理方式，请参阅下一章关于 [中间件](./advanced-middleware.md) 的内容。