...
FastAPI 使用参数定义函数来声明 API 端点接收的输入。这些函数不仅用于声明，还处理数据验证、序列化以及为 OpenAPI 自动生成文档。

当你需要为参数声明元数据或验证时，可以在 `typing.Annotated` 中使用这些函数。
...
| `convert_underscores` | `bool` | 如果为 `True`（默认值），则会将参数名称中的下划线 `_` 转换成连字符 `-` 来查找标头。 |
| `alias` | `str` | 参数的备用名称。 |
| `title` | `str` | 参数的可读标题。 |
| `description` | `str` | 可读的描述。 |
| `gt` | `float` | 值必须大于此值。 |
...
`Cookie` 与 `Query` 和 `Header` 共享大部分相同的验证和元数据参数，例如 `default`、`alias`、`title`、`description`、数值验证（`gt`、`ge` 等）以及字符串验证（`min_length`、`max_length` 等）。
...
| `embed` | `bool` | 如果为 `True`，则参数将被期望在一个 JSON 正文中，并以其参数名作为键。如果声明了多个 `Body` 参数，这会自动发生。 |
